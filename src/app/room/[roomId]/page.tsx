...
import { Mic, MicOff, Video, VideoOff, Users, Volume2, MessageCircle, Send, X, VolumeX, UserPlus, UserMinus, WifiOff, RefreshCw } from "lucide-react";

// ── Constants ────────────────────────────────────────────────────────────────

const PROXIMITY_RANGE = 300;
const EMOTE_RANGE = 80;
const EMOTES = ["😂","🎉","❤️","🔥","👏","😮","😢","🤔","👍","👋","💡","✨"];
...
// ── Helper Component: PeerRow ─────────────────────────────────────────────────

function PeerRow({
  peer, emoji, micOn, camOn, unread, onChat,
}: {
  peer: Peer;
  emoji: string;
  micOn: boolean;
    ...
    <div className="flex items-center gap-3 p-2 rounded-lg bg-gray-700/30 hover:bg-gray-700/50 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1 text-xs text-gray-400">
          {micOn ? <Mic className="w-3 h-3 text-green-400" /> : <MicOff className="w-3 h-3 text-gray-600" />}
          {camOn ? <Video className="w-3 h-3 text-green-400 ml-1" /> : <VideoOff className="w-3 h-3 text-gray-600 ml-1" />}
        </div>
      </div>
...
// ── Helper Component: NotificationToast ──────────────────────────────────────

function NotificationToast({ message, emoji, onDone }: {
  message: string; emoji: string; onDone: () => void;
}) {
  useEffect(() => { const t = setTimeout(onDone, 3200); return () => clearTimeout(t); }, [onDone]);
  return (
    <div className="flex items-center gap-3 bg-gray-800/90 backdrop-blur border border-gray-600/50 text-white px-4 py-3 rounded-xl shadow-2xl animate-in fade-in slide-in-from-top-3 max-w-xs">
...
export default function RoomPage() {
  ...
  // Remote video refs (one per peer)
  const peerVideoRefs = useRef<Map<string, HTMLVideoElement | null>>(new Map());

  // ── Initialise emoji ────────────────────────────────────────────────────────

  useEffect(() => {
    if (!userEmoji) setUserEmoji(getRandomEmoji());
  }, [userEmoji]);

  // ── Add tracks helper (fix: late stream after connection) ──────────────────
  ...
  const addTracksToPeer = useCallback((pc: RTCPeerConnection, peerId: string) => {
    ...
    if (anyAdded) console.log(`[webrtc] tracks added to existing peer ${peerId}`);
  }, []);
  ...
  const createPeer = useCallback((peerId: string, isInitiator: boolean) => {
    ...
    if (isInitiator) {
      pc.createOffer()
        ...
        .catch(e => console.error(`[webrtc] offer failed for ${peerId}:`, e));
    }

    return pc;
  }, [roomId, addTracksToPeer, audioUnlocked]);

  // ── Periodically re-add tracks to peers (handles late stream) ──────────────

  useEffect(() => {
    const id = setInterval(() => {
      pcByPeer.current.forEach((pc, peerId) => addTracksToPeer(pc, peerId));
    }, 1000);
  ...
  useEffect(() => {
    ...
    navigator.mediaDevices
      .getUserMedia({ audio: true, video: camParam })
      .then(stream => {
  ...
  useEffect(() => {
    ...
    // Existing peers list (sent only to newcomer)
    ...
    // New peer joined (broadcast to others)
    socket.on("peer-joined", (peer: Peer) => {
      console.log("[client] peer joined:", peer);
      setPeers(prev => [...prev, peer]);
      setPeerEmojis(prev => {
        const next = new Map(prev);
        if (!next.has(peer.id)) next.set(peer.id, getRandomEmoji());
        return next;
      });
      addNotification(`${peer.displayName} entrou na sala`, peer.id);
    });

    // Peer left
    socket.on("peer-left", ({ id }: { id: string }) => {
      const peer = peers.find(p => p.id === id);
      addNotification(`${peer?.displayName || "Alguém"} saiu da sala`, id, true);
      ...
      peerVideoRefs.current.delete(id);
      ...
      if (data.sdp) {
        if (data.sdp.type === "offer") {
          try {
            ...
            if (queued?.length) { for (const c of queued) try { await pc.addIceCandidate(new RTCIceCandidate(c)); } catch {} pendingCandidates.current.set(from, []); }
    ...
    socket.on("private-message", ({ from, message, timestamp, fromId }: { from: string; message: string; timestamp: number; fromId: string }) => {
      setPrivateMessages(prev => {
        const next = new Map(prev);
        const list = next.get(fromId) || [];
        next.set(fromId, [...list, { id: `${fromId}-${timestamp}`, from, fromId: "peer", message, timestamp }]);
        return next;
      });
  ...
  // ── Canvas render loop (movement + emojis + proximity) ─────────────────────
  ...
  useEffect(() => {
    ...
    function frame() {
      setPos(old => {
        ...
        x = Math.max(20, Math.min(el.width - 20, x));
        y = Math.max(20, Math.min(el.height - 20, y));
        return { x, y };
      });

      ctx.clearRect(0, 0, el.width, el.height);

      // Grid background (Gather-style tiled floor)
      ctx.fillStyle = "#1a2332";
      ctx.fillRect(0, 0, el.width, el.height);
      ctx.strokeStyle = "rgba(255,255,255,0.04)";
      ctx.lineWidth = 1;
      const gridSize = 40;
      for (let gx = 0; gx < el.width; gx += gridSize) { ctx.beginPath(); ctx.moveTo(gx, 0); ctx.lineTo(gx, el.height); ctx.stroke(); }
      for (let gy = 0; gy < el.height; gy += gridSize) { ctx.beginPath(); ctx.moveTo(0, gy); ctx.lineTo(el.width, gy); ctx.stroke(); }
      ...
      for (const [id, p] of peerPos.current.entries()) {
        const d = Math.hypot(pos.x - p.x, pos.y - p.y);
        const vol = calcSpatialVolume(d);
        const audio = remoteAudioByPeer.current.get(id);
        if (audio && audioUnlocked) { audio.volume = vol; audio.muted = vol === 0; }
        if (vol > 0) newNearby.add(id);
      }
      setNearbyIds(newNearby);

      // Draw remote peers
      for (const [id, p] of peerPos.current.entries()) {
        const peer = peers.find(pp => pp.id === id);
        const emoji = peerEmojis.get(id);
        const d = Math.hypot(pos.x - p.x, pos.y - p.y);
        const nearby = newNearby.has(id);
        const conn = connectionStatus.get(id);

        // Connection indicator
        if (conn === "connecting") {
          ctx.fillStyle = "rgba(251,191,36,0.3)"; ctx.beginPath(); ctx.arc(p.x, p.y, 38, 0, Math.PI * 2); ctx.fill();
        } else if (conn === "disconnected") {
          ctx.fillStyle = "rgba(239,68,68,0.25)"; ctx.beginPath(); ctx.arc(p.x, p.y, 35, 0, Math.PI * 2); ctx.fill();
        } else if (nearby) {
          ctx.fillStyle = "rgba(99,102,241,0.12)"; ctx.beginPath(); ctx.arc(p.x, p.y, 42, 0, Math.PI * 2); ctx.fill();
        }
        ...
        if (peer) {
          ...
          ctx.beginPath(); ctx.roundRect(p.x - bw / 2, p.y - 28, bw, bh, 8); ctx.fill();
          ctx.fillStyle = nearby ? "#a5b4fc" : "#ffffff";
          ctx.fillText(peer.displayName, p.x, p.y - 28 + bh / 2);
        }
      }

      // Draw local avatar + name with proximity ring
      {
        ...
        ctx.font = "11px Arial";
        const tw = ctx.measureText(displayName || "Você").width;
        const pad = 6, bw = tw + pad * 2, bh = 18;
        ctx.fillStyle = "rgba(59,130,246,0.85)";
        ctx.beginPath(); ctx.roundRect(pos.x - bw / 2, pos.y - 28, bw, bh, 8); ctx.fill();
        ctx.fillStyle = "#ffffff";
        ctx.fillText(displayName || "Você", pos.x, pos.y - 28 + bh / 2);
      }
  ...
  function handleDblClick(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    setPos({ x: (e.clientX - rect.left) * scaleX, y: (e.clientY - rect.top) * scaleY });
    playEmote();
  }

  // ── Click canvas → unlock audio ─────────────────────────────────────────────
  ...
  // ── Spontaneous emote on proximity change ───────────────────────────────────

  useEffect(() => {
    if (!userEmoji || !audioUnlocked) return;
    const nearby = Array.from(nearbyIds);
    if (nearby.length > 0 && Math.random() < 0.03) playEmote();
  }, [nearbyIds, userEmoji, audioUnlocked]);

  function playEmote() {
    if (!userEmoji) return;
    const emoji = EMOTES[Math.floor(Math.random() * EMOTES.length)];
    addNotification(`Você reagiu ${emoji}`, userEmoji, true);
  }

  // ── Notifications ──────────────────────────────────────────────────────────

  function addNotification(message: string, _id: string | number, isLocal = false) {
    const id = Date.now().toString() + Math.random();
    const peer = peers.find(p => p.id === String(_id));
    setNotifications(prev => [...prev.slice(-3), { id, message, emoji: isLocal ? userEmoji : (peerEmojis.get(String(_id)) || "👋") }]);
  }
  ...
  return (
    <div className="absolute inset-0 min-h-screen w-full bg-gradient-to-br from-gray-900 via-black to-gray-800 full-screen-bg">
        ...
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-white font-semibold text-sm">Sala: {String(roomId)}</h1>
                <p className="text-gray-400 text-xs">iTalk</p>
              </div>
            </div>
            <span className="text-xs text-gray-400 hidden sm:inline">
              {peersCount + 1} participantes
            </span>
          </div>
          <div className="flex items-center gap-3">
            {!audioUnlocked && (
              <button onClick={unlockAudio} className="flex items-center gap-2 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors text-sm relative z-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-4 flex-1">
          ...
          <div className="bg-gray-800/30 backdrop-blur-sm rounded-2xl border border-gray-700/50 overflow-hidden flex flex-col">
            <div className="px-3 py-2 border-b border-gray-700/50 flex items-center justify-between text-xs text-gray-400">
              <div>
                Conexões: {diag.connected} | Áudio: {diag.remoteAudio} | {nearbyIds.size} próximos
              </div>
              <div className="text-gray-500">
                WASD mover • Clique ativar áudio • Duplo clique teleportar
              </div>
            </div>
              ...
              <div className="space-y-1.5">
                ...
                {peers.map(p => (
                  <PeerRow
                    key={p.id}
                    peer={p}
                    emoji={peerEmojis.get(p.id) || "🤖"}
                    micOn={micEnabled}
                    camOn={camEnabled}
                    unread={unreadPrivateCounts.get(p.id)}
                    onChat={openPrivateChat}
                  />
                ))}
                {peers.length === 0 && (
                  <p className="text-center text-gray-500 text-sm py-4">Apenas você na sala</p>
                )}
              </div>
              ...
              {nearbyIds.size > 0 && (
                <div className="mt-4 pt-3 border-t border-gray-700/50">
                  <p className="text-xs text-gray-400 mb-2 flex items-center gap-1">
                    <Volume2 className="w-3 h-3" /> Perto de você (áudio espacial ativo)
                  </p>
                  <div className="flex flex-wrap gap-1.5">
                    {Array.from(nearbyIds).map(id => {
                      const p = peers.find(pp => pp.id === id);
                      if (!p) return null;
                      const d = peerPos.current.get(id);
                      if (!d) return null;
                      const dist = Math.round(Math.hypot(pos.x - d.x, pos.y - d.y));
                      return (
                        <span key={id} className="inline-flex items-center gap-1 px-2 py-1 bg-blue-900/40 border border-blue-500/30 rounded-lg text-xs text-blue-200">
                          <span>{peerEmojis.get(id)}</span> <span className="truncate max-w-[80px]">{p.displayName}</span>
                          <span className="text-blue-400">{dist}px</span>
                        </span>
                      );
                    })}
                  </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-800 rounded-2xl border border-gray-700/50 w-full max-w-md h-[600px] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-gray-700/50">
              ...
              <button onClick={() => setShowChat(false)} className="text-gray-400 hover:text-white transition-colors"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="p-4 border-t border-gray-700/50">
              <div className="flex gap-2">
                <input type="text" value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyPress={handleKeyPress}
                  placeholder="Digite sua mensagem..." className="flex-1 bg-gray-700 text-white placeholder-gray-400 px-3 py-2 rounded-lg border border-gray-600 focus:border-blue-500 focus:outline-none text-sm" />
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-800 rounded-2xl border border-gray-700/50 w-full max-w-md h-[600px] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-gray-700/50">
              ...
              <button onClick={closePrivateChat} className="text-gray-400 hover:text-white transition-colors"><X className="w-5 h-5" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
            ...
            <div className="p-4 border-t border-gray-700/50">
              <div className="flex gap-2">
                <input type="text" value={newPrivateMessage} onChange={e => setNewPrivateMessage(e.target.value)} onKeyPress={handlePrivateKeyPress}
                  placeholder={`Mensagem para ${selectedPeer.displayName}...`} className="flex-1 bg-gray-700 text-white placeholder-gray-400 px-3 py-2 rounded-lg border border-gray-600 focus:border-purple-500 focus:outline-none text-sm" />
      ...
      <div className="fixed top-20 right-4 z-50 flex flex-col gap-2 pointer-events-none">
        {notifications.map(n => (
          <NotificationToast key={n.id} message={n.message} emoji={n.emoji} onDone={() => setNotifications(prev => prev.filter(x => x.id !== n.id))} />
        ))}
      </div>

      {/* Hidden container for remote audio/video */}
